const app = {
  ...
  soundEnabled: JSON.parse(localStorage.getItem('grafik_sound')) !== false,
  audioCtx: null,
  ...
  toggleSound() {
    ...
    localStorage.setItem('grafik_sound', this.soundEnabled);
  ...
  checkTF(p, c) { if(p === c) { this.playSound('success'); this.score++; this.playTF(); } else { this.playSound('fail'); this.endGame('Bad Render!', `Puan: ${this.score}`); } },
  ...
  guess(l, w, b) {
    ...
    if(!curr.includes('_')) { this.playSound('win'); this.saveProgress('game'); this.endGame('Smooth Animation!', `Terim: ${w}`); this.triggerConfetti(); }
  ...
  endGame(t, m) {
    ...
    if(t.includes('Smooth') || t.includes('Bad') || t.includes('End')) this.playSound('win');
